...
DELTA["Ui"].Name = "LazzyExecutorIcon"
...
DELTA["8"]["Text"] = [[Access Lazzy Executor through completing the key system, doesn't take long!]];
...
DELTA["c0"]["Text"] = "";
DELTA["c0"]["PlaceholderText"] = "Thank you for using Lazzy Executor <3\nLazzy Executor UI";
...
DELTA["fb"]["Text"] = [[Select your desired Lazzy Executor theme below]];
DELTA["fb"]["Name"] = [[Desc]];
...
DELTA["194"]["Text"] = [[Lazzy Executor Console]];
...
DELTA["195"]["Text"] = [[Console that provides output and input from the executor API.]];
...
--// LAZZY EXECUTOR UI ENHANCEMENT LAYER
--// Visual-only. Core execution, key, HTTP and API logic is unchanged.
pcall(function()
    local TweenService = game:GetService("TweenService")
    local UIS = game:GetService("UserInputService")
    local Root = DELTA["1"]

    local Accent = Color3.fromRGB(45, 145, 255)
    local Panel = Color3.fromRGB(16, 19, 28)
    local Panel2 = Color3.fromRGB(23, 27, 39)
    local Text = Color3.fromRGB(245, 248, 255)
    local Muted = Color3.fromRGB(145, 156, 178)

    local function addCorner(obj, radius)
        if obj:IsA("GuiObject") and not obj:FindFirstChild("LazzyCorner") then
    ...
    local function addStroke(obj)
        if obj:IsA("GuiObject") and not obj:FindFirstChild("LazzyStroke") then
    ...
    local function addGradient(obj)
        if obj:IsA("GuiObject") and not obj:FindFirstChild("LazzyGradient") then
            local g=Instance.new("UIGradient")
            g.Name="LazzyGradient"
            g.Color=ColorSequence.new(Panel2,Panel)
            g.Rotation=25
            g.Parent=obj
        end
    end
    ...
    local function bindHover(btn)
        if not btn:IsA("GuiButton") or btn:GetAttribute("LazzyHover") then return end
        btn:SetAttribute("LazzyHover",true)
        local original=btn.BackgroundTransparency
        btn.MouseEnter:Connect(function()
            pcall(function()
                TweenService:Create(btn,TweenInfo.new(.12,Enum.EasingStyle.Quad),{
                    BackgroundTransparency=math.max(0,original-.08)
                }):Play()
            end)
        end)
    ...
    for _,obj in ipairs(Root:GetDescendants()) do
        if obj:IsA("Frame") then
            ...
            if n=="keysystem" or n=="sidebar" or n=="executor" or n=="scripthub"
            or n=="settings" or n=="console" or n=="popup" or n=="home" then
    ...
    if DELTA["DaIcon"] then
        DELTA["DaIcon"].BackgroundColor3=Color3.fromRGB(13,16,24)
        addCorner(DELTA["DaIcon"],14)
        if DELTA["daStroke"] then
            DELTA["daStroke"].Color=Accent
            DELTA["daStroke"].Thickness=2
        end
        bindHover(DELTA["DaIcon"])
    end

    if not Root:FindFirstChild("LazzyBrandBadge") then
        local badge=Instance.new("Frame")
        badge.Name="LazzyBrandBadge"
        badge.Position=UDim2.new(0,16,0,12)
        badge.Size=UDim2.new(0,180,0,42)
        badge.BackgroundColor3=Panel2
        badge.BackgroundTransparency=.08
        badge.ZIndex=5000
        badge.Parent=Root
        addCorner(badge,13)
        addStroke(badge)
        ...
        title.Size=UDim2.new(1,-26,0,19)
        title.Font=Enum.Font.GothamBold
        title.Text="LAZZY EXECUTOR"
        ...
        sub.Position=UDim2.new(0,13,0,24)
        sub.Size=UDim2.new(1,-26,0,12)
    ...
    if UIS.TouchEnabled then
        local badge=Root:FindFirstChild("LazzyBrandBadge")
        if badge then
            badge.Size=UDim2.new(0,165,0,40)
            badge.Position=UDim2.new(0,9,0,9)
        end
    end
end)
